...
IGNORE_ENV_PROXY = os.getenv("NETWORK_IGNORE_ENV_PROXY", "true").lower() in {
    "1",
    "true",
    "yes",
    "on",
}
FORCE_PROXY_ONLY = os.getenv("NETWORK_FORCE_PROXY_ONLY", "true").lower() in {
    "1",
    "true",
    "yes",
    "on",
}
FORCE_PROXY_URL = os.getenv("NETWORK_FORCE_PROXY_URL", "").strip()


def _apply_env_proxy_policy() -> None:
...
def _request(method: str, url: str, **kwargs: Any) -> requests.Response:
    _apply_env_proxy_policy()
    with requests.Session() as session:
        session.trust_env = not IGNORE_ENV_PROXY
...
def get_proxy_from_pool() -> dict[str, str] | None:
    forced = _normalize_proxy(FORCE_PROXY_URL)
    if forced:
        return {"http": forced, "https": forced}

    if USE_PROXY_POOL:
        params = {k: v[0] for k, v in parse_qs(PROXY_POOL_PARAMS).items()}
        try:
            resp = request_get(PROXY_POOL_API, params=params, timeout=5)
    ...
    if LOCAL_PROXY_URL:
        proxy = _normalize_proxy(LOCAL_PROXY_URL)
        return {"http": proxy, "https": proxy}
    if FORCE_PROXY_ONLY:
        raise RuntimeError(
...
def push_to_backend(rows: list[dict[str, Any]]) -> None:
    if not rows:
        return
    ingest_resp = request_post(INGEST_URL, json=rows, timeout=15)
    ingest_resp.raise_for_status()
    inserted = ingest_resp.json().get("inserted", 0)
    print(f"inserted listings: {inserted}")

    if AUTO_SCAN_AFTER_INGEST:
        scan_resp = request_post(AUTO_SCAN_URL, params={"limit": 200}, timeout=20)
