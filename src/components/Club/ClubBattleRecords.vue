        ...
        <div ref="exportDom" v-else-if="battleRecords && battleRecords.roleDetailsList" class="records-list">
          ...
          <div v-for="(member, index) in battleRecords.roleDetailsList" :key="member.roleId" class="member-card">
            <div class="member-header">
              ...
              <n-button text class="details-button" size="small" @click="toggleMemberDetails(member.roleId)">
                <template #icon>
                  <n-icon>
                    <ChevronDown v-if="!expandedMembers.has(member.roleId)"></ChevronDown>
            ...
            <n-collapse-transition :show="expandedMembers.has(member.roleId)">
              <div class="battle-details">
                <div v-if="member.targetRoleList && member.targetRoleList.length > 0" class="battles-list">
                  ...
                  >
                    <div class="battle-participants">
                      ...
                      <div class="battle-vs">
                        <n-tag size="small" :type="battle.attackType === 0 ? 'warning' : 'info'">
                          {{
                            parseAttackType(battle.attackType) }}
                        </n-tag>
                        <n-tag size="small" :type="battle.newWinFlag === 2 ? 'success' : 'error'">
                          {{
                ...
                <div v-else class="no-battles">
                  <n-empty description="暂无战斗记录" size="small"></n-empty>
                </div>
              </div>
