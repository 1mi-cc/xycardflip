...
class VnpySystem:
    ...
    def start(self) -> dict[str, str | bool]:
        ...
        if not self.data_engine.keywords:
            defaults = [kw.strip() for kw in settings.monitor_keywords if kw and kw.strip()]
            if not defaults:
                fallback = settings.monitor_keyword.strip()
                if fallback:
                    defaults = [fallback]
            if defaults:
                self.data_engine.set_keywords(defaults)
    ...
    def status(self) -> dict:
        cookie_info = {"cookie_present": False, "cookie_has_token": False, "cookie_error": ""}
        provider = getattr(self.main_engine, "_cookie_provider", None)
        if provider is not None:
            try:
                cookie_value = provider.get_cookie()
                cookie_info["cookie_present"] = bool(cookie_value)
                cookie_info["cookie_has_token"] = "_m_h5_tk" in (cookie_value or "")
                cookie_info["cookie_error"] = getattr(provider, "last_error", "") or ""
            except Exception as exc:  # pragma: no cover
                cookie_info["cookie_error"] = str(exc)
        return {
