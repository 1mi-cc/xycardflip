...
class XianyuClient:
    ...
    def __init__(self) -> None:
        ...
        self.keyword = (
            settings.monitor_keywords[0]
            if settings.monitor_keywords
    ...
    def _fetch_mtop_search(
        ...
        resp = request_get(mtop_url, params=params, headers=headers, timeout=12, proxies=proxies)
        if resp.status_code != 200:
            raise XianyuHttpError(resp.status_code, resp.text[:200])
        ...
        if ret and not any("SUCCESS" in str(r) for r in ret):
            # Surface mtop auth/limit errors to caller instead of timing out on HTML fallback.
            raise XianyuHttpError(401, "; ".join(str(r) for r in ret)[:200])
